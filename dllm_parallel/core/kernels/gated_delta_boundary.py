...
"""FLA-compatible Gated DeltaNet recurrence with differentiable boundaries.

This module is derived from flash-linear-attention 0.5.2's
``fla.ops.common.chunk_delta_h``.  It preserves the original recurrence and
adds two operations needed by block-diffusion training: selected FP32 state
outputs in forward and gradient injection at those states in backward.
"""
...
@triton.heuristics({
    ...
    'STORE_BOUNDARY_STATES': lambda args: args['boundary_states'] is not None,
...
def chunk_gated_delta_rule_fwd_kernel_h_blockdim64(
    ...
    STORE_BOUNDARY_STATES: tl.constexpr,
    ...
    for i_t in range(NT):
        ...
        if STORE_BOUNDARY_STATES:
            i_boundary = tl.load(boundary_slots + i_t)
            if i_boundary >= 0:
                p_boundary = boundary_states + (
                    (i_n * NB + i_boundary) * HV + i_h
                ).to(tl.int64) * K * V
                if STATE_V_FIRST:
                    p_boundary1 = p_boundary + o_v[:, None] * K + o_k1[None, :]
                    m_boundary1 = m_v[:, None] & m_k1[None, :]
                else:
                    p_boundary1 = p_boundary + o_k1[:, None] * V + o_v[None, :]
                    m_boundary1 = m_k1[:, None] & m_v[None, :]
                tl.store(p_boundary1, b_h1, mask=m_boundary1)
        if K > 64:
            if STATE_V_FIRST:
            ...
            if STORE_BOUNDARY_STATES:
                ...
                if i_boundary >= 0:
                    ...
                    if STATE_V_FIRST:
                        p_boundary2 = p_boundary + o_v[:, None] * K + o_k2[None, :]
                        m_boundary2 = m_v[:, None] & m_k2[None, :]
                    else:
                        p_boundary2 = p_boundary + o_k2[:, None] * V + o_v[None, :]
                        m_boundary2 = m_k2[:, None] & m_v[None, :]
                    tl.store(p_boundary2, b_h2, mask=m_boundary2)
        if K > 128:
            if STATE_V_FIRST:
            ...
            if STORE_BOUNDARY_STATES:
                ...
                if i_boundary >= 0:
                    ...
                    if STATE_V_FIRST:
                        p_boundary3 = p_boundary + o_v[:, None] * K + o_k3[None, :]
                        m_boundary3 = m_v[:, None] & m_k3[None, :]
                    else:
                        p_boundary3 = p_boundary + o_k3[:, None] * V + o_v[None, :]
                        m_boundary3 = m_k3[:, None] & m_v[None, :]
                    tl.store(p_boundary3, b_h3, mask=m_boundary3)
        if K > 192:
            if STATE_V_FIRST:
            ...
            if STORE_BOUNDARY_STATES:
                ...
                if i_boundary >= 0:
                    ...
                    if STATE_V_FIRST:
                        p_boundary4 = p_boundary + o_v[:, None] * K + o_k4[None, :]
                        m_boundary4 = m_v[:, None] & m_k4[None, :]
                    else:
                        p_boundary4 = p_boundary + o_k4[:, None] * V + o_v[None, :]
                        m_boundary4 = m_k4[:, None] & m_v[None, :]
                    tl.store(p_boundary4, b_h4, mask=m_boundary4)
...
@triton.heuristics({
    ...
    'USE_BOUNDARY_STATE_GRADIENTS': lambda args: args['boundary_state_grads'] is not None,
    'IS_VARLEN': lambda args: args['cu_seqlens'] is not None,
})
@fla_cache_autotune(
    configs=[
...
def chunk_gated_delta_rule_bwd_kernel_dhu_blockdim64(
    ...
    USE_BOUNDARY_STATE_GRADIENTS: tl.constexpr,
    STATE_V_FIRST: tl.constexpr,
    IS_VARLEN: tl.constexpr,
):
    pid = tl.program_id(0)
    ...
    for i_t in range(NT - 1, -1, -1):
        ...
        # A target block consumes the state before chunk i_t. Its gradient is
        # therefore injected after differentiating chunk i_t, so it propagates
        # only through the clean prefix preceding that boundary.
        if USE_BOUNDARY_STATE_GRADIENTS:
            i_boundary = tl.load(boundary_slots + i_t)
            if i_boundary >= 0:
                p_boundary = boundary_state_grads + (
                    (i_n * NB + i_boundary) * HV + i_h
                ).to(tl.int64) * K * V
                if STATE_V_FIRST:
                    p_boundary1 = p_boundary + o_v[:, None] * K + o_k1[None, :]
                    ...
                    p_boundary1 = p_boundary + o_k1[:, None] * V + o_v[None, :]
                    m_boundary1 = m_k1[:, None] & m_v[None, :]
                b_dh1 += tl.load(p_boundary1, mask=m_boundary1, other=0.0)
                if K > 64:
                    if STATE_V_FIRST:
                        p_boundary2 = p_boundary + o_v[:, None] * K + o_k2[None, :]
                        m_boundary2 = m_v[:, None] & m_k2[None, :]
                        ...
                        p_boundary2 = p_boundary + o_k2[:, None] * V + o_v[None, :]
                        m_boundary2 = m_k2[:, None] & m_v[None, :]
                    b_dh2 += tl.load(p_boundary2, mask=m_boundary2, other=0.0)
                if K > 128:
                    if STATE_V_FIRST:
                        p_boundary3 = p_boundary + o_v[:, None] * K + o_k3[None, :]
                        m_boundary3 = m_v[:, None] & m_k3[None, :]
                        ...
                        p_boundary3 = p_boundary + o_k3[:, None] * V + o_v[None, :]
                        m_boundary3 = m_k3[:, None] & m_v[None, :]
                    b_dh3 += tl.load(p_boundary3, mask=m_boundary3, other=0.0)
                if K > 192:
                    if STATE_V_FIRST:
                        p_boundary4 = p_boundary + o_v[:, None] * K + o_k4[None, :]
                        m_boundary4 = m_v[:, None] & m_k4[None, :]
                        ...
                        p_boundary4 = p_boundary + o_k4[:, None] * V + o_v[None, :]
                        m_boundary4 = m_k4[:, None] & m_v[None, :]
                    b_dh4 += tl.load(p_boundary4, mask=m_boundary4, other=0.0)
...
def chunk_gated_delta_rule_fwd_h_with_boundaries(
    k: torch.Tensor,
    w: torch.Tensor,
    u: torch.Tensor,
    ...
    boundary_slots: torch.IntTensor | None = None,
    ...
    if boundary_slots is not None:
        if boundary_slots.dtype != torch.int32 or boundary_slots.device != k.device:
            raise ValueError("boundary_slots must be a CUDA int32 tensor colocated with k")
        if boundary_slots.numel() != NT:
            raise ValueError("boundary_slots must contain one entry per recurrent chunk")
        if num_boundaries <= 0:
            raise ValueError("num_boundaries must be positive when boundary_slots are provided")
        state_shape = (B, int(num_boundaries), HV, V, K) if state_v_first else (B, int(num_boundaries), HV, K, V)
        boundary_states = torch.empty(state_shape, device=k.device, dtype=torch.float32)
    else:
        if num_boundaries:
            raise ValueError("num_boundaries requires boundary_slots")
    ...
    return h, v_new, final_state, boundary_states


def chunk_gated_delta_rule_bwd_dhu_with_boundaries(
    q: torch.Tensor,
    k: torch.Tensor,
    w: torch.Tensor,
    do: torch.Tensor,
    ...
    chunk_size: int = 64,
    chunk_indices: torch.LongTensor | None = None,
    boundary_state_grads: torch.Tensor | None = None,
    boundary_slots: torch.IntTensor | None = None,
    ...
    if (boundary_state_grads is None) != (boundary_slots is None):
        raise ValueError("boundary gradients and slots must be provided together")
    if boundary_state_grads is not None:
        if boundary_state_grads.dtype != torch.float32:
            raise ValueError("boundary state gradients must be FP32")
        if boundary_slots.dtype != torch.int32 or boundary_slots.device != q.device:
            raise ValueError("boundary_slots must be a CUDA int32 tensor colocated with q")
        if boundary_slots.numel() != NT:
            raise ValueError("boundary_slots must contain one entry per recurrent chunk")
        num_boundaries = int(boundary_state_grads.shape[1])
    else:
        num_boundaries = 0
    ...
    chunk_gated_delta_rule_bwd_kernel_dhu_blockdim64[grid](
        ...
        gk=gk,
        dht=dht,
        boundary_state_grads=boundary_state_grads,
        boundary_slots=boundary_slots,
        dh0=dh0,
        do=do,
        dh=dh,
