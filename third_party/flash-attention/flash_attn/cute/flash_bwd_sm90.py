...
class FlashAttentionBackwardSm90:
    ...
    def __init__(
        ...
        if pack_gqa:
            assert qhead_per_kvhead > 1, "Pack-GQA requires grouped-query attention"
            assert tile_m % qhead_per_kvhead == 0, (
                "Pack-GQA requires tile_m to be divisible by the query-head group size"
            )
        ...
        # Pack-GQA block metadata is expressed directly in the virtual packed
        # query coordinate system. It therefore uses the same sparse-to-MMA
        # subtile factor as the kernel instead of replicating every physical
        # query tile across the query-head group.
        self.query_subtile_factor = subtile_factor
    ...
    def __call__(
        ...
        if const_expr(self.pack_gqa):
            # The preprocess zeros the ordinary [B, Hq, S*D] allocation. Reinterpret
            # those same bytes as [B, Hkv, (S*G)*D] so the main kernel can accumulate
            # virtual packed rows without a physical query pack or a dQ scatter.
            if const_expr(cute.rank(mdQaccum.shape) == 3):
                packed_flat = mdQaccum.shape[2] * self.qhead_per_kvhead
                mdQaccum = cute.make_tensor(
                    mdQaccum.iterator,
                    cute.make_layout(
                        (mdQaccum.shape[0], nheads_kv, packed_flat),
                        stride=(mdQaccum.stride[0], packed_flat, 1),
                    ),
                )
            else:
                packed_flat = mdQaccum.shape[1] * self.qhead_per_kvhead
                mdQaccum = cute.make_tensor(
                    mdQaccum.iterator,
                    cute.make_layout(
                        (nheads_kv, packed_flat),
                        stride=(packed_flat, 1),
                    ),
                )
        ...
        self.tma_copy_bytes["LSE"] = (
            0 if self.pack_gqa else self.tile_m * Float32.width // 8
        )
        ...
        tma_atom_Q, tma_tensor_Q = make_q_tma_atom(
        ...
        tma_atom_dO, tma_tensor_dO = make_q_tma_atom(
            cpasync.CopyBulkTensorTileG2SOp(),
            mdO_og if const_expr(self.pack_gqa) else mdO,
            cute.select(self.sdO_layout, mode=[0, 1]),
            (self.tile_m, self.tile_hdimv),
        )
        tma_atom_LSE = tma_atom_dPsum = None
        tma_tensor_LSE, tma_tensor_dPsum = mLSE, mdPsum
        if const_expr(self.qhead_per_kvhead == 1 or self.pack_gqa):
            mdK_tma = (
        ...
        tile_sched_args = TileSchedulerArguments(
            ...
            1,  # num_splits
            (
                cute.size(mQ.shape[0]) // self.qhead_per_kvhead
                if const_expr(self.pack_gqa)
                else cute.size(mQ.shape[0])
            ),  # physical query length
            mQ.shape[1],  # headdim
            ...
            mSeqUsedQ=mSeqUsedK,
            # Backward schedules KV tiles; packing changes the inner Q traversal,
            # not the number of KV work tiles.
    ...
    def kernel(
        ...
        SeqlenInfoCls = partial(
            SeqlenInfoQK.create,
            seqlen_q_static=(
                mQ.shape[0][1] if const_expr(self.pack_gqa) else mQ.shape[0]
            ),
        ...
        if warp_idx_in_wg == 0:
            ...
            while work_tile.is_valid_tile:
                ...
                gLSE = (
                    mLSE_cur
                    if const_expr(self.pack_gqa)
                    else cute.local_tile(mLSE_cur, (self.tile_m,), (None,))
                )
                gdPsum = (
                    mdPsum_cur
                    if const_expr(self.pack_gqa)
                    else cute.local_tile(mdPsum_cur, (self.tile_m,), (None,))
                )
                ...
                if const_expr(self.pack_gqa):
                    ...
                    load_LSE = copy_utils.tma_producer_copy_fn(
                        copy_utils.cpasync_bulk_get_copy_fn(gLSE, sLSE),
                        pipeline_Q,
                    )
                    load_dPsum = copy_utils.tma_producer_copy_fn(
                        copy_utils.cpasync_bulk_get_copy_fn(gdPsum, sdPsum),
                        pipeline_dO,
                    )
                ...
                if const_expr(not self.use_block_sparsity):
                    ...
                    total_m_block_cnt = get_total_q_block_count_bwd(
                        ...
                        head_idx,
                        n_block,
                        subtile_factor=self.query_subtile_factor,
    ...
    def dQaccum_store(
        ...
        while work_tile.is_valid_tile:
            ...
            if const_expr(not seqlen.has_cu_seqlens_q):
                ...
                if const_expr(self.pack_gqa):
                    padded_offset_q *= self.qhead_per_kvhead
