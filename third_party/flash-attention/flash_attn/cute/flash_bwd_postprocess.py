...
# Wheel-level capability contract consumed by Turbo-dLLM's production
# preflight. The packed-GQA dQ layout retains allocator alignment after its
# head-to-sequence transformation.
PACK_GQA_DQACCUM_ALIGNMENT_PROPAGATED = True
PACK_GQA_DQACCUM_RELAYOUT_ALIGNMENT_PROPAGATED = True
...
class FlashAttentionBackwardPostprocess:
    def __init__(
        ...
        self.pack_gqa = qhead_per_kvhead > 1
    ...
    def __call__(
        ...
        if const_expr(self.pack_gqa):
            nheads_q = mdQ.shape[-2]
            nheads_kv = nheads_q // self.qhead_per_kvhead
            ...
            # ``cute.make_tensor`` creates a new tensor descriptor and drops
            # the iterator's compile-time alignment fact. Restore it after
            # the packed-GQA relayout for Hopper's 128-bit async-copy atom.
            mdQaccum = assume_tensor_aligned(mdQaccum)
            mdQ = layout_utils.select(
                mdQ,
                [1, 3, 2, 0] if cute.rank(mdQ.shape) == 4 else [0, 2, 1],
            )
            mdQ = pack_gqa_layout(
                mdQ,
        ...
        if const_expr(mCuSeqlensQ is not None):
            ...
            num_head = mdQ.shape[2] if const_expr(self.pack_gqa) else mdQ.shape[1]
            num_batch = mCuSeqlensQ.shape[0] - 1
            num_block = cute.ceil_div(
                mdQ.shape[0][1] * self.qhead_per_kvhead
                if const_expr(self.pack_gqa)
                else mdQ.shape[0],
                self.tile_m,
            )
            ...
            num_batch = mdQ.shape[3] if const_expr(self.pack_gqa) else mdQ.shape[0]
            num_block = cute.ceil_div(
                mdQ.shape[0][1] * self.qhead_per_kvhead
                if const_expr(self.pack_gqa)
                else mdQ.shape[1],
    ...
    def kernel(
        ...
        if work_tile.is_valid_tile:
            ...
            seqlen = SeqlenInfoQK.create(
                batch_idx,
                mdQ.shape[0][1] if const_expr(self.pack_gqa) else mdQ.shape[1],
                0,
                mCuSeqlensQ=mCuSeqlensQ,
                mCuSeqlensK=None,
                mSeqUsedQ=mSeqUsedQ,
            ...
            if const_expr(self.pack_gqa):
                ...
                # Packing GQA heads into the sequence axis preserves the
                # allocator alignment: every head and batch offset is a
                # multiple of the rounded query tile times the head width.
                # Carry that invariant through the layout transformation so
                # the postprocess can retain its vectorized async load.
            ...
            gdQ = (
                None
                if const_expr(self.pack_gqa)
                else cute.local_tile(mdQ_cur, (self.tile_m, self.tile_hdim), (m_block, 0))
            )
