            ...
            if const_expr(not self.swap_AB):
                ...
                if const_expr(self.qhead_per_kvhead_packgqa != 1):
                    for r in cutlass.range(cute.size(tScS_mn.shape[0]), unroll_full=True):
                        for c in cutlass.range(cute.size(tScS_mn.shape[1]), unroll_full=True):
                            packed_row = (
                                m_block * self.tile_m + tScS_mn[r, c][ROW]
                            )
                            row_idx = packed_row // self.qhead_per_kvhead_packgqa
                            col_idx = n_block * self.tile_n + tScS_mn[r, c][COL]
                            out_of_bounds = cutlass.Boolean(False)
                            if const_expr(mask_seqlen):
                                out_of_bounds = cutlass.Boolean(
                                    (row_idx >= self.seqlen_q) | (col_idx >= self.seqlen_k)
                                )
                            causal_offset = self.seqlen_k - self.seqlen_q
                            if const_expr(mask_causal):
                                masked = cutlass.Boolean(col_idx > row_idx + causal_offset)
                            else:
                                masked = cutlass.Boolean(False)
                                if const_expr(self.window_size_right is not None):
                                    masked |= cutlass.Boolean(
                                        col_idx
                                        > row_idx
                                        + causal_offset
