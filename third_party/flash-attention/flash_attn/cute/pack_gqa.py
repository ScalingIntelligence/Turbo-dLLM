...
class PackGQA:
    ...
    def load_stat(
        ...
        """Gather one virtual Pack-GQA tile of scalar query statistics."""

        tidx = cute.arch.thread_idx()[0] % cute.arch.WARP_SIZE
        rows_per_thread = cute.ceil_div(self.m_block_size, cute.arch.WARP_SIZE)
        for row_group in cutlass.range_constexpr(rows_per_thread):
            row = row_group * cute.arch.WARP_SIZE + tidx
            idx = block * self.m_block_size + row
            if row < self.m_block_size and idx < seqlen * self.qhead_per_kvhead:
                seq_idx = idx // self.qhead_per_kvhead
                query_head = idx - seq_idx * self.qhead_per_kvhead
                sStat[row] = mStat[((query_head, seq_idx),)]
        cute.arch.sync_warp()
    ...
    def load_stat_stage(
        ...
        mStat: cute.Tensor,
        sStat: cute.Tensor,
        src_idx: cutlass.Int32,
        producer_state,
        seqlen: cutlass.Int32,
    ):
        self.load_stat(
            mStat,
            sStat[None, producer_state.index],
            src_idx,
            seqlen,
        )
