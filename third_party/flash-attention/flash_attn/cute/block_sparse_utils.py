...
def _load_q_do_block_sm90(
    ...
    # Pack-GQA stages scalar statistics synchronously; issue the TMA copy last
    # so its completion cannot release the consumer before those stores finish.
...
def consume_block_sparse_mma_bwd_sm90(
    ...
    Returns updated pipeline states and whether any query block contributed to
    the current K/V tile.
    """
    q_cnt, q_idx, full_cnt, full_idx, *_ = blocksparse_tensors
    curr_q_cnt = q_cnt[batch_idx, head_idx, n_block]
    curr_q_idx = q_idx[batch_idx, head_idx, n_block, None]
