...
def _validate_pack_gqa_sparse_metadata(
    tensors: BlockSparseTensorsTorch,
) -> None:
    head_dim = 0 if tensors.mask_block_cnt.ndim == 2 else 1
    for name, tensor in (
        ...
        if tensor is not None and tensor.shape[head_dim] != 1:
            raise ValueError(
                "Native Pack-GQA backward requires every block-sparse metadata "
                f"tensor to be head-broadcast; {name}.shape[{head_dim}]="
                f"{tensor.shape[head_dim]}"
            )

    has_full_blocks = tensors.full_block_cnt is not None or tensors.full_block_idx is not None
    if has_full_blocks and (
        tensors.full_block_cnt is None or tensors.full_block_idx is None
    ):
        raise ValueError("full_block_cnt and full_block_idx must be provided together")


def _validate_pack_gqa_backward_capability(
    *,
    arch: int,
    head_dim: int,
    head_dim_v: int,
    ...
    if arch // 10 != 9:
        raise NotImplementedError("Native Pack-GQA backward requires SM90")
    if qhead_per_kvhead <= 1:
        raise ValueError("pack_gqa=True requires grouped-query attention")
    if head_dim != head_dim_v:
        raise ValueError(
            "Native Pack-GQA backward requires equal query/key and value head dimensions"
        )
    if tile_m % qhead_per_kvhead != 0:
        raise ValueError(
            "Native Pack-GQA backward requires the selected query tile "
            f"({tile_m}) to be divisible by the GQA group ({qhead_per_kvhead})"
        )
    if deterministic:
        raise NotImplementedError("Deterministic native Pack-GQA backward is not implemented")
...
def _flash_attn_fwd(
    ...
    if use_block_sparsity:
        ...
        if pack_gqa and block_sparse_tensors.mask_block_cnt.shape[head_dim_idx] != 1:
            raise ValueError(
                "Pack-GQA requires head-independent block-sparse metadata; "
                "set pack_gqa=False for head-dependent masks"
            )
        if cu_seqlens_q is not None:
...
def _compile_bwd_postprocess(
    ...
    use_2cta_instrs, cluster_size, arch, qhead_per_kvhead,
):
...
def _bwd_postprocess_convert(
    ...
    atom_layout, swap_ab,
    use_2cta_instrs=False, cluster_size=1, qhead_per_kvhead=1,
):
...
def _flash_attn_bwd(
    ...
    if pack_gqa is None:
        pack_gqa = qhead_per_kvhead > 1
    if pack_gqa:
        _validate_pack_gqa_backward_capability(
            arch=arch,
            head_dim=head_dim,
        ...
        if block_sparse_tensors is not None:
            _validate_pack_gqa_sparse_metadata(block_sparse_tensors)
    ...
    # Generic GQA uses dK/dV accum+postprocess because query heads launch independently.
    # Native Pack-GQA schedules one CTA per KV head and reduces its query-head group
    # in registers, so dK/dV use the direct owner store.
    # hd=256 2CTA backward has its own internal postprocess for dK/dV.
    dKV_postprocess = (
        qhead_per_kvhead > 1 and not pack_gqa and not use_dedicated_hd256_kernel
    )
    if dKV_postprocess:
    ...
    normalized_block_sparse_tensors = None
    if block_sparse_tensors is not None:
        sparse_num_head = num_head_kv if pack_gqa else num_head
        sparse_seqlen_q = (
            seqlen_q * qhead_per_kvhead if pack_gqa else seqlen_q
        )
        (
            normalized_block_sparse_tensors,
            block_sparse_broadcast_pattern,
            ...
            block_sparse_tensors,
            batch_size=batch_size,
            num_head=sparse_num_head,
            seqlen_q=sparse_seqlen_q,
            seqlen_k=seqlen_k,
    ...
    if not use_dedicated_hd256_kernel:
        ...
        _bwd_postprocess_convert(
            ...
            qhead_per_kvhead=qhead_per_kvhead if pack_gqa else 1,
        )
