...
template <int Stages, class ClusterShape_, class TileShape_MNK_, int kHeadDimV, class Element_, class ElementAccum_, class ArchTag_,
        ...
        bool Is_bdlm_=false, bool Is_dflash_=false>
struct CollectiveMainloopFwdSm90 {
    ...
    using BlockMN_t = flash::BlockMN<SeqlenInfo_t, kBlockM, kBlockN, Is_causal, Is_local, PackGQA, Split, Is_dflash>;
    ...
    mma(Params const& params,
        ...
        flash::Mask<kBlockM, kBlockN, PackGQA, TiledMmaQK, false, Is_dflash> mask(
        ...
        if constexpr (IntraWGOverlap) {
            ...
            auto no_mask_fn = [&](auto& tSrS, int n_block) {
                if constexpr (Is_dflash) {
                    mask.template apply<false /*Seqlenk_mask*/, false /*Causal_mask*/, false /*Local_mask*/>(
                        tSrS, m_block, n_block);
                } else if (params.bdlm_query_blocks != nullptr) {
                    mask.template apply<false /*Seqlenk_mask*/, false /*Causal_mask*/, false /*Local_mask*/>(
                        tSrS, m_block, n_block);
                }
            };
            ...
            } else if (params.bdlm_query_blocks != nullptr && params.bdlm_key_start < 0) {
                #pragma unroll 1
                for (; n_block >= n_block_min_before_local_mask; --n_block) {
                    fwd_step(n_block, no_mask_fn, cute::true_type{} /*check_inf*/);
                }
