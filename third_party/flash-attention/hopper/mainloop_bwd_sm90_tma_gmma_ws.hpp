...
template <int Stages, int Stages_dO, int Stages_dS, class ClusterShape_, class TileShape_MNK_, class Element_, class ElementAccum_, class ArchTag_,
        ...
        bool Mma_dP_is_RS=false, bool Is_bdlm_=false, bool Is_dflash_=false>
struct CollectiveMainloopBwdSm90 {
    ...
    static constexpr bool Is_bdlm = Is_bdlm_;
    ...
    to_underlying_arguments(Arguments const& args) {
        ...
        return {args.shape_Q, args.shape_K, 
                ...
                args.dflash_context_starts, args.dflash_context_stops,
                args.dflash_anchor_valid,
                args.dflash_context_starts_batch_stride,
                args.dflash_context_stops_batch_stride,
                args.dflash_anchor_valid_batch_stride,
                args.dflash_block_size, args.dflash_key_start};
    }
        ...
        if (lane_predicate) {
            ...
            for (; m_block < m_block_max - 1; ++m_block) {
                ...
                if (params.ptr_dLSE != nullptr) {
                    copy(bulk_copy.with(*pipeline_do.producer_get_barrier(smem_pipe_write_do_cur)),
                         gdLSE(_, m_block), sdLSE(_, smem_pipe_write_do_cur.index()));
                }
    ...
    store_dq(Params const& params,
        ...
        if ((params.bdlm_query_blocks != nullptr || Is_dflash) && m_block_max <= m_block_min) { return; }
    ...
    mma(Params const& params,
        ...
        auto [m_block_min, m_block_max] = BlockMN_t::get_m_block_min_max(
            ...
            params.dflash_anchor_valid_batch_stride, params.dflash_block_size, params.dflash_key_start);
        // It's possible to have m_block_max <= m_block_min. Exit early
        if ((params.bdlm_query_blocks != nullptr || Is_dflash) && m_block_max <= m_block_min) { return false; }
        if constexpr (Is_causal || Is_local || Varlen) {
        ...
        Tensor tLSEsdLSE = cute::conditional_return<!SdP_swapAB>(
            group_modes<0, 2>(thread_mma_SdP.partition_C(sdLSEMma)(make_coord(_0{}, _, _0{}), _, _0{}, _)),
            group_modes<0, 3>(thread_mma_SdP.partition_C(sdLSEMma)(make_coord(_, _0{}, _), _0{}, _, _)));
        ...
        auto bwd_step = [&](int m_block, auto mask_fn) {
            ...
            Tensor tLSErdLSE = cute::conditional_return<!ShuffledPsum>(make_fragment_like(tLSEsdLSE(_, _0{})), make_tensor<ElementAccum>(Int<kStatsPerThread>{}));
            if constexpr (!ShuffledPsum) {
                cute::copy(tLSEsdPsum(_, smem_pipe_read_do_cur.index()), tLSErdPsum);
                if (params.ptr_dLSE != nullptr) {
                    cute::copy(tLSEsdLSE(_, smem_pipe_read_do_cur.index()), tLSErdLSE);
                } else {
                    clear(tLSErdLSE);
                }
            } else {
                ...
                for (int i = 0; i < kStatsPerThread; ++i) {
                    ...
                    tLSErdLSE(i) = params.ptr_dLSE != nullptr
                        ? tLSEsdLSE((thread_idx % 32) / 4 + i * 8, smem_pipe_read_do_cur.index())
                        : ElementAccum(0.0f);
                }
            }
