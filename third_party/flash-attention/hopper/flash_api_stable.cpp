...
namespace {
int bdlm_output_dtype_tag(torch::headeronly::ScalarType dtype) {
    if (dtype == torch::headeronly::ScalarType::Half) { return 0; }
    if (dtype == torch::headeronly::ScalarType::BFloat16) { return 1; }
    if (dtype == torch::headeronly::ScalarType::Float) { return 2; }
    STD_TORCH_CHECK(false, "bdlm_fwd_accum output dtype must be fp16, bf16, or fp32");
}
} // anonymous namespace
...
mha_fwd(Tensor q,   // (b, s_q, h, d) or (total_q, h, d) if there is cu_seqlens_q
        ...
        std::optional<bool> pack_gqa_,
        int64_t sm_margin,
        std::optional<Tensor> bdlm_query_blocks_,  // (s_q) or (b, s_q), int32
        std::optional<Tensor> bdlm_query_is_clean_,  // (s_q) or (b, s_q), bool
    ...
    if (is_bdlm) {
        STD_TORCH_CHECK(bdlm_query_blocks_.has_value() && bdlm_query_is_clean_.has_value(),
                    "BDLM attention requires both bdlm_query_blocks and bdlm_query_is_clean");
        STD_TORCH_CHECK(!is_varlen_q && !is_varlen_k && !paged_KV,
                    "BDLM FlashAttention path currently supports fixed non-paged Q/K/V only");
        STD_TORCH_CHECK(!is_causal && window_size_left < 0 && window_size_right < 0 && attention_chunk == 0,
                    "BDLM FlashAttention path must not be combined with causal/local/attention_chunk masks");
        STD_TORCH_CHECK(num_splits == 1, "BDLM FlashAttention path does not support split-KV");
        STD_TORCH_CHECK(bdlm_block_size > 0, "bdlm_block_size must be positive");
        ...
        STD_TORCH_CHECK(bdlm_query_blocks.scalar_type() == torch::headeronly::ScalarType::Int, "bdlm_query_blocks must have dtype torch.int32");
        STD_TORCH_CHECK(bdlm_query_is_clean.scalar_type() == torch::headeronly::ScalarType::Bool, "bdlm_query_is_clean must have dtype torch.bool");
        STD_TORCH_CHECK((bdlm_query_blocks.dim() == 1 && bdlm_query_blocks.size(0) >= seqlen_q) ||
        ...
        STD_TORCH_CHECK((bdlm_query_is_clean.dim() == 1 && bdlm_query_is_clean.size(0) >= seqlen_q) ||
    ...
    if (is_bdlm) {
        ...
        params.bdlm_query_blocks_ptr = static_cast<int const*>(bdlm_query_blocks.data_ptr());
        params.bdlm_query_is_clean_ptr = static_cast<bool const*>(bdlm_query_is_clean.data_ptr());
...
std::tuple<Tensor, Tensor, Tensor, Tensor, Tensor> mha_bwd(
    ...
    int64_t bdlm_block_size,
    int64_t bdlm_key_start,
    std::optional<Tensor> softmax_lse_grad_,
    int64_t bdlm_clean_offset
) {
    ...
    if (is_bdlm) {
        ...
        STD_TORCH_CHECK(!is_varlen_q && !is_varlen_k,
                    "BDLM FlashAttention backward currently supports fixed Q/K/V only");
        STD_TORCH_CHECK(!is_causal && window_size_left < 0 && window_size_right < 0,
    ...
    if (softmax_lse_grad_.has_value()) {
        ...
        STD_TORCH_CHECK(softmax_lse_grad.scalar_type() == torch::headeronly::ScalarType::Float, "softmax_lse_grad must have dtype torch.float32");
    ...
    bool const is_structured_mask = is_causal || is_local || is_bdlm || softcap > 0.0 || softmax_lse_grad_.has_value();
...
void bdlm_fwd_accum(
    ...
    Tensor m,
    Tensor l,
    std::optional<Tensor> query_indices_,
    Tensor bdlm_query_blocks,
    Tensor bdlm_query_is_clean,
    int64_t bdlm_block_size,
    int64_t bdlm_key_start,
    ...
    STD_TORCH_CHECK(q.scalar_type() == k.scalar_type() && q.scalar_type() == v.scalar_type(),
                    "q, k, v must have the same dtype");
    STD_TORCH_CHECK(q.scalar_type() == torch::headeronly::ScalarType::Half ||
                    q.scalar_type() == torch::headeronly::ScalarType::BFloat16,
                    "bdlm_fwd_accum expects fp16 or bf16 q/k/v");
    STD_TORCH_CHECK(numerator.scalar_type() == torch::headeronly::ScalarType::Float,
                    "numerator must be fp32");
    STD_TORCH_CHECK(m.scalar_type() == torch::headeronly::ScalarType::Float, "m must be fp32");
    STD_TORCH_CHECK(l.scalar_type() == torch::headeronly::ScalarType::Float, "l must be fp32");
    STD_TORCH_CHECK(bdlm_query_blocks.scalar_type() == torch::headeronly::ScalarType::Int,
                    "bdlm_query_blocks must be int32");
    STD_TORCH_CHECK(bdlm_query_is_clean.scalar_type() == torch::headeronly::ScalarType::Bool,
                    "bdlm_query_is_clean must be bool");
    STD_TORCH_CHECK(q.dim() == 4 && k.dim() == 4 && v.dim() == 4,
                    "q, k, v must be [B, S, H, D]");
    STD_TORCH_CHECK(numerator.dim() == 4 && m.dim() == 3 && l.dim() == 3,
                    "numerator must be [B,H,Q,D] and m/l must be [B,H,Q]");
    STD_TORCH_CHECK(numerator.size(0) == q.size(0) && numerator.size(1) == q.size(2) &&
    ...
    CHECK_SHAPE(l, numerator.size(0), numerator.size(1), numerator.size(2));
    ...
    if (compact) {
        ...
        STD_TORCH_CHECK(query_indices.scalar_type() == torch::headeronly::ScalarType::Long,
                        "query_indices must be int64");
        STD_TORCH_CHECK(query_indices.dim() == 1, "query_indices must be [compact_Q]");
        STD_TORCH_CHECK(q.size(1) == query_indices.size(0),
                        "compact q length must match query_indices");
        STD_TORCH_CHECK(!initial,
                        "compact bdlm_fwd_accum initial mode would leave skipped rows undefined");
    } else {
        STD_TORCH_CHECK(q.size(1) == numerator.size(2),
...
void boxed_mha_fwd(
    ...
    auto bdlm_query_blocks = to<std::optional<Tensor>>(stack[34]);
    auto bdlm_query_is_clean = to<std::optional<Tensor>>(stack[35]);
    auto bdlm_block_size = to<int64_t>(stack[36]);
    auto bdlm_key_start = to<int64_t>(stack[37]);
    auto bdlm_clean_offset = to<int64_t>(stack[38]);

    auto [out_, softmax_lse, out_accum, softmax_lse_accum] = mha_fwd(q, k, v, k_new, v_new, q_v, out, cu_seqlens_q, cu_seqlens_k, cu_seqlens_k_new, seqused_q, seqused_k, max_seqlen_q, max_seqlen_k, page_table, kv_batch_idx, leftpad_k, rotary_cos, rotary_sin, seqlens_rotary, q_descale, k_descale, v_descale, softmax_scale, is_causal, window_size_left, window_size_right, attention_chunk, softcap, is_rotary_interleaved, scheduler_metadata, num_splits, pack_gqa, sm_margin, bdlm_query_blocks, bdlm_query_is_clean, bdlm_block_size, bdlm_key_start, bdlm_clean_offset);
...
void boxed_mha_bwd(
    ...
    auto bdlm_query_blocks = to<std::optional<Tensor>>(stack[22]);
    auto bdlm_query_is_clean = to<std::optional<Tensor>>(stack[23]);
    auto bdlm_block_size = to<int64_t>(stack[24]);
    auto bdlm_key_start = to<int64_t>(stack[25]);
    auto softmax_lse_grad = to<std::optional<Tensor>>(stack[26]);
    auto bdlm_clean_offset = to<int64_t>(stack[27]);

    auto [softmax_d, softmax_lse_log2, dq_accum, dk_accum, dv_accum] = mha_bwd(dout, q, k, v, out, softmax_lse, dq, dk, dv, cu_seqlens_q, cu_seqlens_k, seqused_q, seqused_k, max_seqlen_q, max_seqlen_k, softmax_scale, is_causal, window_size_left, window_size_right, softcap, deterministic, sm_margin, bdlm_query_blocks, bdlm_query_is_clean, bdlm_block_size, bdlm_key_start, softmax_lse_grad, bdlm_clean_offset);
...
void boxed_bdlm_fwd_accum(
    ...
    auto k = to<Tensor>(stack[1]);
    auto v = to<Tensor>(stack[2]);
    auto numerator = to<Tensor>(stack[3]);
    auto m = to<Tensor>(stack[4]);
    auto l = to<Tensor>(stack[5]);
    auto query_indices = to<std::optional<Tensor>>(stack[6]);
    auto bdlm_query_blocks = to<Tensor>(stack[7]);
    auto bdlm_query_is_clean = to<Tensor>(stack[8]);
    auto bdlm_block_size = to<int64_t>(stack[9]);
    auto bdlm_key_start = to<int64_t>(stack[10]);
    auto bdlm_clean_offset = to<int64_t>(stack[11]);
    auto softmax_scale = to<std::optional<double>>(stack[12]);
    auto initial = to<bool>(stack[13]);
...
STABLE_TORCH_LIBRARY(flash_attn_3, m) {
    m.def("fwd("
        ...
        "int bdlm_clean_offset = 0) -> (Tensor(out!), Tensor, Tensor, Tensor)");
    m.def("bwd("
        "Tensor dout,"
        "Tensor q,"
        "Tensor k,"
        ...
        "int bdlm_clean_offset = 0) -> (Tensor, Tensor, Tensor, Tensor, Tensor)");
    m.def("fwd_combine("
        "Tensor out_partial,"
        "Tensor lse_partial,"
        "Tensor(out!)? out = None,"
...
STABLE_TORCH_LIBRARY_IMPL(flash_attn_3, CUDA, m) {
    ...
    m.impl("bdlm_fwd_accum", &boxed_bdlm_fwd_accum);
