...
template <int Stages, int Stages_dO, class TileShape_MNK_, class Element_, class ElementAccum_, class ArchTag_,
        ...
        bool V_in_regs=false, bool Is_dflash_=false>
struct CollectiveMainloopBwdSm80 {
    ...
    static constexpr bool Is_dflash = Is_dflash_;
    ...
    using BlockMN_t = flash::BlockMN<SeqlenInfo_t, kBlockM, kBlockN, Is_causal, Is_local, false, false, Is_dflash>;
    ...
    struct TensorStorageSharedQV : cute::aligned_struct<128> {
        ...
        cute::array_aligned<ElementAccum, cute::cosize_v<SmemLayoutLSE>, 128> smem_dlse;
        SmemP_t smem_p;
    ...
    struct Arguments {
        ...
        float const* const ptr_dLSE;
        StrideLSE const stride_dLSE;
    ...
    to_underlying_arguments(Arguments const& args) {
        ...
        return {args.ptr_Q, args.shape_Q, args.stride_Q,
                ...
                args.ptr_dLSE, args.stride_dLSE,
                args.softmax_scale,
                ...
                args.bdlm_query_blocks, args.bdlm_query_is_clean,
                args.bdlm_query_blocks_batch_stride, args.bdlm_query_is_clean_batch_stride,
                args.bdlm_block_size, args.bdlm_key_start, args.bdlm_clean_offset,
                args.dflash_context_starts, args.dflash_context_stops, args.dflash_anchor_valid,
                args.dflash_context_starts_batch_stride, args.dflash_context_stops_batch_stride,
                args.dflash_anchor_valid_batch_stride, args.dflash_block_size, args.dflash_key_start};
    }
    ...
    mma(Params const& params,
        ...
        auto m_block_min_max = BlockMN_t::get_m_block_min_max(
            ...
            params.bdlm_query_blocks, params.bdlm_query_is_clean,
            params.bdlm_query_blocks_batch_stride, params.bdlm_query_is_clean_batch_stride,
            params.bdlm_block_size, params.bdlm_key_start, params.bdlm_clean_offset,
            params.dflash_context_starts, params.dflash_context_stops, params.dflash_anchor_valid,
            params.dflash_context_starts_batch_stride, params.dflash_context_stops_batch_stride,
            params.dflash_anchor_valid_batch_stride, params.dflash_block_size, params.dflash_key_start);
        ...
        if ((params.bdlm_query_blocks != nullptr || Is_dflash) && m_block_max <= m_block_min) {
            return false;
        }
        ...
        Tensor sdLSE = make_tensor(make_smem_ptr(shared_storage.tensors.mainloop.smem_dlse.data()), SmemLayoutLSE{});
        ...
        Tensor sdLSEMma = make_tensor(make_smem_ptr(shared_storage.tensors.mainloop.smem_dlse.data()), SmemLayoutLSEMma{});
        ...
        Tensor mdLSE = make_tensor(make_gmem_ptr(params.ptr_dLSE), params.shape_LSE, params.stride_dLSE)(_, bidh, !is_varlen_q ? bidb : 0);
        ...
        Tensor gdLSE = local_tile(domain_offset(make_coord(seqlen_info.offset_q_padded), mdLSE), select<0>(TileShape_MNK{}), make_coord(_));  // (M, _)
        ...
        Tensor tLSEgdLSE = gmem_thr_copy_lse.partition_S(gdLSE);
        Tensor tLSEsdLSE = gmem_thr_copy_lse.partition_D(sdLSE);
        ...
        Tensor tSsdLSEMma = logical_divide(thr_mma_SdP.partition_C(sdLSEMma), Shape<_2>{});
        Tensor tSsdLSE = group_modes<0, 2>(cute::conditional_return<!SdP_swapAB>(
            tSsdLSEMma(make_coord(_0{}, _), _, _0{}, _),  // (2, MMA_M, PIPE)
            tSsdLSEMma(make_coord(_, _0{}), _0{}, _, _)));  // (2, MMA_N, PIPE)
        ...
        flash::Mask<kBlockM, kBlockN, false /*PackGQA*/, TiledMmaSdP, SdP_swapAB, Is_dflash> mask(
            thread_idx, seqlen_q, seqlen_k, params.window_size_left, params.window_size_right, 0 /*sink_token_length*/,
            params.attention_chunk_divmod, params.qhead_per_khead_divmod,
            params.bdlm_query_blocks, params.bdlm_query_is_clean,
            params.bdlm_query_blocks_batch_stride, params.bdlm_query_is_clean_batch_stride,
            bidb, params.bdlm_block_size, params.bdlm_key_start,
            params.bdlm_clean_offset,
            params.dflash_context_starts, params.dflash_context_stops, params.dflash_anchor_valid,
            params.dflash_context_starts_batch_stride, params.dflash_context_stops_batch_stride,
            params.dflash_anchor_valid_batch_stride, params.dflash_block_size,
            params.dflash_key_start
        );
        ...
        auto load_dO_dPsum = [&] (int const m_block, int const smem_pipe_write) {
            ...
            Tensor tLSEgdLSE_cur = tLSEgdLSE(_, _, m_block);
            Tensor tLSEsdLSE_cur = tLSEsdLSE(_, _, smem_pipe_write);
            #pragma unroll
            for (int m = 0; m < size<1>(tLSEsdPsum); ++m) {
                if (get<0>(tLSEcLSE(_0{}, m)) < kBlockM) {
                    cute::copy(gmem_tiled_copy_lse, tLSEgdPsum_cur(_, m), tLSEsdPsum_cur(_, m));
                    if (params.ptr_dLSE != nullptr) {
                        cute::copy(gmem_tiled_copy_lse, tLSEgdLSE_cur(_, m), tLSEsdLSE_cur(_, m));
                    }
                }
            }
        };
        ...
        auto bwd_step = [&](int m_block, auto mask_fn) {
            ...
            Tensor tLSErdLSE = cute::conditional_return<!ShuffledPsum>(make_fragment_like(tSsdLSE(_, _0{})), make_tensor<ElementAccum>(Int<kStatsPerThread>{}));
            if constexpr (!ShuffledPsum) {
                if (params.ptr_dLSE != nullptr) {
                    cute::copy(tSsdLSE(_, kStages_dO > 1 ? smem_pipe_read_do_cur : 0), tLSErdLSE);
                } else {
                    clear(tLSErdLSE);
                }
            } else {
                #pragma unroll
                for (int i = 0; i < kStatsPerThread; ++i) {
                    tLSErdLSE(i) = params.ptr_dLSE != nullptr
                        ? tSsdLSE((thread_idx % 32) / 4 + i * 8, kStages_dO > 1 ? smem_pipe_read_do_cur : 0)
                        : ElementAccum(0.0f);
                }
            }
            ...
            for (int mi = 0; mi < size<0>(dS); ++mi) {
                ...
                float const dLSE_cur = [&] {
                    if constexpr (!ShuffledPsum) return tLSErdLSE(mi);
                    else return __shfl_sync(0xffffffff, tLSErdLSE(mi / 8), (mi % 8) * 4 + (thread_idx % 4));
                }();
                #pragma unroll
                for (int ni = 0; ni < size<1>(dS); ++ni) {
                    dS(mi, ni) = scores(mi, ni) * (dS(mi, ni) - dP_sum_cur + dLSE_cur);
