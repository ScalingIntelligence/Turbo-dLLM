...
class BDLMFlashAttnFunc(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx,
        q,
        ...
        v = v.contiguous() if v.stride(-1) != 1 and v.stride(-3) != 1 else v
        query_blocks = query_blocks.contiguous()
        query_is_clean = query_is_clean.contiguous()
        out, softmax_lse, _, _ = flash_attn_3_gpu.fwd(
            q,
            k,
            v,
            ...
            bdlm_block_size=int(block_size),
            bdlm_key_start=int(key_start),
            bdlm_clean_offset=int(clean_offset),
        )
        ctx.save_for_backward(q, k, v, out, softmax_lse, query_blocks, query_is_clean)
    ...
    def backward(ctx, dout, dlse):
        q, k, v, out, softmax_lse, query_blocks, query_is_clean = ctx.saved_tensors
        dq, dk, dv = torch.empty_like(q), torch.empty_like(k), torch.empty_like(v)
        dlse = None if dlse is None else dlse.contiguous()
        flash_attn_3_gpu.bwd(
            maybe_contiguous(dout),
            q,
            k,
            v,
            out,
            ...
            bdlm_query_blocks=query_blocks,
            bdlm_query_is_clean=query_is_clean,
            bdlm_block_size=ctx.block_size,
            bdlm_key_start=ctx.key_start,
            bdlm_clean_offset=ctx.clean_offset,
...
def bdlm_flash_attn_func(
    ...
    """BDLM block-masked FlashAttention over fixed-shape Q/K/V.

    q, k, v use FlashAttention's layout: (batch, seqlen, heads, headdim).
    query_blocks and query_is_clean are either (seqlen) or (batch, seqlen).
    """
    out, softmax_lse = BDLMFlashAttnFunc.apply(
        q,
        k,
        v,
        query_blocks,
