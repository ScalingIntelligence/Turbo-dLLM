...
from ._cute.seqlen_info import SeqlenInfo
...
class BDLMSplitDBwdPreprocess:
  ...
  def kernel(
    ...
    if work_tile.is_valid_tile:
      ...
      # With PDL enabled, signal that dependent kernels may start once O/dO loads are done.
      # With PDL disabled, ordinary stream ordering keeps the following kernels after all writes below.
      ...
      if tOcO[0, 0, 0][1] == 0:
        for m in cutlass.range(cute.size(PdP_sum), unroll_full=True):
          row = tOcO[0, m, 0][0]
          PdPsum_val = 0.0
          # A ring shard may contain no legal keys for this query row. Its
          # forward LSE is -inf and its output is mathematically zero, but the
          # kernel's unused output accumulator need not be finite. Do not form
          # O*dO for that row: P is identically zero, so D and dLSE are both
          # irrelevant and the exact backward contribution is zero.
          if row < seqlen_limit and lse != -Float32.inf:
