...
from ._cute.named_barrier import NamedBarrierBwd


class BDLMSplitDBwdDKDVSm90:
  ...
  def kernel(
    ...
    if warp_idx == 0:
      for atom in [
        tma_atom_Q, tma_atom_dO, tma_atom_K, tma_atom_V, tma_atom_dK,
        tma_atom_dV
      ]:
  ...
  def load(
    ...
    while work_tile.is_valid_tile:
      ...
      m_block_count = block_info.get_m_block_count(seqlen, n_block)
      process_tile = const_expr(
        not self.is_varlen_q
        and not self.precomputed_tile_bounds
        and not self.precomputed_tile_worklist
      ) or m_block_count > 0
      ...
      if process_tile:
        ...
        for d_pass in cutlass.range_constexpr(self.num_d_passes):
          ...
          for q_head_offset in cutlass.range(self.qhead_per_kvhead, unroll=1):
            ...
            for i_m in cutlass.range(m_block_count, unroll=1):
              m_block = block_info.get_m_block(seqlen, n_block, i_m)
              ...
              for d_inner in cutlass.range_constexpr(self.num_d_inner):
                ...
                if cutlass.const_expr(d_inner == self.num_d_inner - 1):
                  load_LSE(
                    src_idx=m_block,
                    dst_idx=producer_state_A.index,
                    tma_bar_ptr=pipeline_A.
                    producer_get_barrier(producer_state_A),
                  )
                pipeline_A.producer_commit(producer_state_A)
                did_produce_A = Boolean(True)
                producer_state_A.advance()

              # ═══ Phase 3: dO chunks → sA. Load dO_d_pass last so WG1 can
              # retain that stage for dV while the producer reuses the other stage
              # for Q_d_pass. dPsum is piggybacked on the last load. ═══
              for d_load_idx in cutlass.range_constexpr(self.num_d_inner):
                d_inner = (d_pass + 1 + d_load_idx) % self.num_d_inner
                ...
                if cutlass.const_expr(d_load_idx == self.num_d_inner - 1):
                  pipeline_A.producer_acquire(
                    producer_state_A,
              ...
              gQ_pass = cute.local_tile(
                mQ_cur, (self.tile_m, self.d_chunk), (None, d_pass)
              )
        ...
        for d_pass in cutlass.range_constexpr(self.num_d_passes):
          ...
          for q_head_offset in cutlass.range(self.qhead_per_kvhead, unroll=1):
            ...
            mask_fn = partial(
              ...
              mask_mod=self.mask_mod if const_expr(aux_tensors is not None) else None,
              aux_tensors=aux_tensors,
            )

            for i_m in cutlass.range(m_block_count, unroll=1):
              m_block, tile_is_full = block_info.get_m_block_and_full(
                seqlen, n_block, i_m
              )
  ...
  def _mma_wg1_one_m_block(
    ...
    # ═══ Release dO chunks consumed by WG2, but retain the final
    # dO_d_pass stage for WG1's later dV GEMM. ═══
    smem_idx_dO_pass = Int32(0)
    for d_load_idx in cutlass.range_constexpr(self.num_d_inner):
      pipeline_A.consumer_wait(
        consumer_state_A, pipeline_A.consumer_try_wait(consumer_state_A)
      )
    ...
    # ═══ Phase 5: dV += P^T @ retained dO_d_pass ═══
    mma_dV_fn(
      A_idx=p_stage,
    ...
    with cute.arch.elect_one():
      pipeline_A.consumer_release_w_index(smem_idx_dO_pass)
    ...
    while work_tile.is_valid_tile:
      ...
      if process_tile:
        ...
        self.zero_v_tail_smem_wg2(
          sV_persist=sV_persist, seqlen=seqlen, n_block=n_block, tidx=tidx
        )
    ...
    for d_load_idx in cutlass.range_constexpr(self.num_d_inner):
      ...
      if cutlass.const_expr(d_load_idx == self.num_d_inner - 1):
        ...
        tLSErdPsum = copy_utils.load_s2r(
          tLSEsdPsum[None, consumer_state_A.index]
        )
      with cute.arch.elect_one():
        pipeline_A.consumer_release(consumer_state_A)
      consumer_state_A.advance()
