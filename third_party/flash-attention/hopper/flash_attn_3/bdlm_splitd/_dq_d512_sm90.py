...
class BDLMSplitDBwdDQSm90:
  """SM90 backward dQ kernel (dQ: cooperative dual WG + loop reversal).

    Computes only dQ (no dK/dV). dK/dV is handled by BDLMSplitDBwdDKDVSm90.
  ...
  def load(
    ...
    while work_tile.is_valid_tile:
      ...
      # ═══ loop nest reversed — n_block outer, d_pass inner ═══
      #
      # Per n_block: push Q+K and dO+V as interleaved pairs to avoid a
      # producer/consumer circular wait, then push Kt_front + Kt_back per d_pass
  ...
  def mma_wg1(
    ...
    # Startup credit lets the first producer publish sdS without waiting for a
    # consumer release from a nonexistent previous iteration.
  ...
  def _mma_wg1_one_n_block(
    ...
    # its own 128; combined with WG2's 128 arrivals, the 256-thread barrier fires.
    cute.arch.barrier(
  ...
  def mma_wg2(
    ...
    # 128 arrivals from WG2 and waits only for WG1's own 128 arrivals.
    cute.arch.barrier_arrive(
      barrier_id=int(NamedBarrierBwd.PEmpty),
      number_of_threads=self.num_mma_threads,  # 256
    )
