...
#
# SM90 native Split-D FlashAttention forward for head_dim=512.
#
...
from ._cute.cute_dsl_utils import assume_tensor_aligned
from . import _cute as utils
from ._cute.mask import AttentionMask
from ._cute.softmax import (
  Softmax,
...
from ._mask import BDLMBlockInfo
from ._cute import pipeline as pipeline_custom
from ._cute.pack_gqa import PackGQA, pack_gqa_layout, make_packgqa_tiled_tma_atom
from ._cute.named_barrier import NamedBarrierFwd
from quack.cute_dsl_utils import ParamsBase
from ._cute.tile_scheduler import (
...
class BDLMSplitDFwdSm90:
  """SM90 forward kernel for head_dim=512.
    ...
    Full-D Q/K WGMMA uses 64x64 query/key tiles. Single-stage K/V buffers keep
    the complete D=512 working set within Hopper's shared-memory limit. sV and
    sO share storage because their mainloop and epilogue lifetimes do not
    overlap. Cross-WG sP/sScale handoff uses named barriers.
    """
  ...
  def __init__(
    ...
    self.precomputed_tile_bounds = precomputed_tile_bounds
    self.precomputed_tile_worklist = precomputed_tile_worklist
    self.qk_acc_dtype = Float32
    self.vec_size: cutlass.Constexpr = getattr(
      score_mod, "__vec_size__", 1 if cutlass.const_expr(has_aux_tensors) else 2
    )
    self.arch = Arch.sm_90a

    # K and V use independent descriptors and pipelines. Their pointers may
    # alias, but descriptor sharing is deliberately disabled.
    self.kv_same = kv_same
    assert not kv_same, (
      "_fwd_d512_sm90 requires independent K/V descriptors"
    )
    ...
    # ════════════════════════════════════════════════════════════════════
    # A 64-column K/V tile reaches Hopper's WGMMA floor while keeping the
    # single-stage full-D Q/K/V working set below the SM90 shared-memory limit.
    assert self.tile_n == 64, "Split-D requires tile_n == 64"
  ...
  def __call__(
    ...
    # sK: full-D (tile_n=64 × 512), num_stages_k=2 stages — true K-stage pipeline
    ...
    # sV: (tile_n=64, tile_hdimv=512), num_stages_v=2 stages
    ...
    # sP: K-major (A operand), (tile_m=64, tile_n=64) × stage=num_stages_sP_buf
  ...
  def kernel(
    ...
    block_info = BDLMBlockInfo(
      ...
      precomputed_tile_bounds=self.precomputed_tile_bounds,
      precomputed_tile_worklist=self.precomputed_tile_worklist,
    )
  ...
  def producer(
    ...
    if is_tma_warp:
      ...
      while work_tile.is_valid_tile:
        ...
        n_block_count = block_info.get_n_block_count(seqlen, m_block)
        ...
        if n_block_count > 0:
          ...
          for i_n in cutlass.range(n_block_count, unroll=1):
            n_block = block_info.get_n_block(seqlen, m_block, i_n)
            # K full-D
            gK = cute.local_tile(
  ...
  def mma_wg1(
    ...
    # Use the matching bf16 shared-memory store atom for the epilogue layout.
    ...
    while work_tile.is_valid_tile:
      ...
      if n_block_count > 0:
        ...
        # buffer; acc_S_b is reserved for the fixed pipeline interface.
        for i_n in cutlass.range(n_block_count - 1, unroll=1):
          n_block, tile_is_full = block_info.get_n_block_and_full(
            seqlen, m_block, i_n + Int32(1)
          )
          k_consumer_state, v_consumer_state, pipeline_state_P_producer, pipeline_state_Scale_producer = mma_one_n_block(
            n_block=n_block,
            tile_is_full=tile_is_full,
      ...
      # Empty tiles retain the output/LSE values initialized by the launcher.
  ...
  def _mma_wg1_one_n_block(
    ...
    # ═══ step A: synchronous n64 QK[n_block] into acc_S_cur ═══
    ...
    # ═══ step G: wait V[n], synchronous PV-front, release V[n] ═══
    ...
    while work_tile.is_valid_tile:
      ...
      if n_block_count > 0:
        ...
        v_consumer_state, pipeline_state_P_consumer, pipeline_state_Scale_consumer = mma_one_n_block(
          n_block=block_info.get_n_block(seqlen, m_block, Int32(0)),
          v_consumer_state=v_consumer_state,
        ...
        for i_n in cutlass.range(n_block_count - 1, unroll=1):
          n_block = block_info.get_n_block(
            seqlen, m_block, i_n + Int32(1)
          )
          v_consumer_state, pipeline_state_P_consumer, pipeline_state_Scale_consumer = mma_one_n_block(
            n_block=n_block,
        ...
        v_consumer_state.advance()
      # Empty tiles retain the output initialized by the launcher.
