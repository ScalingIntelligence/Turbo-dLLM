...
"""Native FlashAttention Split-D forward compiler for Hopper head_dim=512."""
...
from ._fwd_d512_sm90 import BDLMSplitDFwdSm90
...
def _splitd_forward_sm90(
  ...
  """Run the repository's Hopper Split-D FlashAttention forward kernel."""
  ...
  if head_dim != 512 or head_dim_v != 512:
    raise ValueError("BDLM Split-D forward requires head_dim == head_dim_v == 512")
  masked = mask_mod is not None or aux_tensors is not None
  expected_metadata = (
    7 if precomputed_tile_worklist
    else (7 if precomputed_tile_bounds else (5 if interval_metadata else 3))
  )
  if masked and (
    mask_mod is None
    or aux_tensors is None
    or len(aux_tensors) != expected_metadata
  ):
    raise ValueError(
      "masked BDLM Split-D forward received an invalid metadata contract"
    )
  ...
  tile_m = SM90_FWD_TILE_M
  tile_n = SM90_FWD_TILE_N

  # The Split-D kernel owns independent K and V shared-memory/TMA pipelines.
  # They remain valid when the two descriptors address the same Gemma K=V
  # tensor; independent descriptors remain correct when their pointers alias.
  kv_same = False
  ...
  # Masked and ragged schedules can skip complete tiles. Prefill their exact
  # mathematical result so no stale accumulator reaches the ring merge.
  if (is_varlen or causal or local or masked) and not is_fake_mode():
    out.zero_()
    if lse is not None:
      lse.fill_(-float("inf"))
  ...
  if compile_key not in _splitd_forward_sm90.compile_cache:
    ...
    # Positional args must match BDLMSplitDFwdSm90.__call__ signature:
    ...
    _splitd_forward_sm90.compile_cache[compile_key] = cute.compile(
      *compile_args,
  ...
  if not is_fake_mode():
    ...
    _call_with_tvm_ffi_current_stream(
      _splitd_forward_sm90.compile_cache[compile_key],
      *call_args,
      device=device,
    )
  return out, lse
...
_splitd_forward_sm90.compile_cache = get_jit_cache("bdlm_fwd_splitd_sm90")
