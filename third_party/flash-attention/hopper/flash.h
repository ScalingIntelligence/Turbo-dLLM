...
struct Flash_fwd_params : public Qkv_params {
    ...
    // BDLM block diffusion mask metadata. When bdlm_query_blocks_ptr is set,
    // Prefix attention uses clean kv_block <= query_block and active
    // kv_block < query_block. Full attention additionally admits the active
    // query's own noisy block and uses bdlm_clean_offset as a global boundary.
    int const * __restrict__ bdlm_query_blocks_ptr;
    bool const * __restrict__ bdlm_query_is_clean_ptr;
    index_t bdlm_query_blocks_batch_stride;
    index_t bdlm_query_is_clean_batch_stride;
    int bdlm_block_size;
    int bdlm_key_start;
    int bdlm_clean_offset;

    // DFlash synthetic query blocks attend to a per-anchor interval in the
    // verifier context. Interval coordinates are global sequence positions.
    int const * __restrict__ dflash_context_starts_ptr;
    int const * __restrict__ dflash_context_stops_ptr;
    bool const * __restrict__ dflash_anchor_valid_ptr;
    index_t dflash_context_starts_batch_stride;
    index_t dflash_context_stops_batch_stride;
    index_t dflash_anchor_valid_batch_stride;
    int dflash_block_size;
    int dflash_key_start;
...
struct Flash_bwd_params : public Flash_fwd_params {
    ...
    void const *__restrict__ softmax_lse_grad_ptr;
    index_t softmax_lse_grad_batch_stride;
    index_t softmax_lse_grad_head_stride;
    index_t softmax_lse_grad_row_stride;
