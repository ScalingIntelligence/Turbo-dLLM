...
template <int Arch, int kHeadDim, int kBlockM, int kBlockN, typename Element,
          ...
          bool V_in_regs=false, bool Is_dflash=false>
void run_flash_bwd(Flash_bwd_params &params, cudaStream_t stream) {
    ...
    using CollectiveMainloop = std::conditional_t<
        ...
        flash::CollectiveMainloopBwdSm90<Stages, Stages_dO, Stages_dS, ClusterShape, TileShape_MNK, Element, ElementAccum, cutlass::arch::Sm90,
            ...
            SdP_swapAB, dKV_swapAB, dQ_swapAB, NumMmaWarpGroups, AtomLayoutMSdP, AtomLayoutNdKV, AtomLayoutMdQ, V_in_regs, false, Is_dflash>,
        flash::CollectiveMainloopBwdSm80<Stages, Stages_dO, TileShape_MNK, Element, ElementAccum, cutlass::arch::Sm80,
            Is_causal, Is_local, Has_softcap, Varlen, Deterministic,
            SdP_swapAB, dKV_swapAB, dQ_swapAB, NumMmaWarpGroups, AtomLayoutMSdP, AtomLayoutNdKV, AtomLayoutMdQ, V_in_regs, Is_dflash>
    >;
    ...
    auto mainloop_args = [&] {
        if constexpr (Arch >= 90) {
            return typename CollectiveMainloop::Arguments {
                ...
                static_cast<float*>(params.dsoftmax_sum),
                {_1{}, seqlen_q_rounded, !is_varlen_q ? params.h * params.seqlen_q_rounded : 0},  // stride_dPsum
                static_cast<float const*>(params.softmax_lse_grad_ptr),
                {_1{}, params.softmax_lse_grad_head_stride, !is_varlen_q ? params.softmax_lse_grad_batch_stride : 0},  // stride_dLSE
                ...
                static_cast<int const*>(params.bdlm_query_blocks_ptr),
                static_cast<bool const*>(params.bdlm_query_is_clean_ptr),
                params.bdlm_query_blocks_batch_stride,
                params.bdlm_query_is_clean_batch_stride,
                params.bdlm_block_size,
                params.bdlm_key_start,
                params.bdlm_clean_offset,
                static_cast<int const*>(params.dflash_context_starts_ptr),
                static_cast<int const*>(params.dflash_context_stops_ptr),
                static_cast<bool const*>(params.dflash_anchor_valid_ptr),
                params.dflash_context_starts_batch_stride,
                params.dflash_context_stops_batch_stride,
                params.dflash_anchor_valid_batch_stride,
                params.dflash_block_size,
...
void run_mha_bwd_dispatch(Flash_bwd_params &params, cudaStream_t stream) {
    BOOL_SWITCH(params.dflash_context_starts_ptr != nullptr, Is_dflash, [&] {
        VARLEN_SWITCH(params.cu_seqlens_q != nullptr || params.cu_seqlens_k != nullptr, Varlen, [&] {
            BOOL_SWITCH(params.h != params.h_k, GQA, [&] {
                BOOL_SWITCH(params.deterministic, Deterministic_, [&] {
                    ...
                    run_flash_bwd<Arch, kHeadDim, kBlockM, kBlockN, T, Is_causal, Is_local, Has_softcap, Varlen /*Varlen*/, Deterministic /*Deterministic*/, GQA, Stages_dO, Stages_dS_or_QSm80, SdP_swapAB, dKV_swapAB, dQ_swapAB, NumMmaWarpGroups, AtomLayoutMSdP, AtomLayoutNdKV, AtomLayoutMdQ, V_in_regs, Is_dflash>(params, stream);
                });
            });
        });
    });
...
void run_mha_bwd_hdim128(Flash_bwd_params &params, cudaStream_t stream) {
    CAUSAL_LOCAL_SWITCH(params.is_causal, params.is_local, Is_causal, Is_local, [&] {
        if constexpr (Arch >= 90) {
            // The extended backward mainloop carries BDLM metadata and optional
            // dLSE state. Its 80x128 variant exceeds SM90's dynamic shared-memory
            // launch limit even when those pointers are null, so head-dim 128
            // consistently uses the valid 64x128 production tile.
